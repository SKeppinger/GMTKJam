...
@export var growable: bool
@export var shrinkable: bool
...
func _on_grow(delta):
	if (growable and not grown) or shrunk:
		targeting = true
...
func _on_shrink(delta):
	if (shrinkable and not shrunk) or grown:
		targeting = true
...
func _on_hitbox_body_entered(body):
	if block_on_grow and grown:
		dying = true
		block_and_die()
	if block_on_default and not grown and not shrunk:
		dying = true
		block_and_die()
